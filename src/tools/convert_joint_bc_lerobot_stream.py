...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Allow image generation from rows without global_state; ports/obstacles will be missing.",
    )
...
def main():
    ...
    missing_global_state_for_images = 0
    total_frames = 0
    total_episodes = 0
    frames_in_episode = 0
    last_step = None
    ...
    for path in expand_inputs(args.input):
        with open(path) as handle:
            for line_no, line in enumerate(handle, 1):
                ...
                if args.include_images:
                    if not row.get("global_state"):
                        if not args.allow_incomplete_images:
                            raise SystemExit(
                                f"{path}:{line_no} has no global_state. Refusing to generate an incomplete "
                                "topdown image without ports/obstacles. Regenerate expert logs with "
                                "include_global_state=True, or pass --allow-incomplete-images for legacy debug data."
                            )
                        missing_global_state_for_images += 1
    ...
    if frames_in_episode:
        ...
        total_episodes += 1
    dataset.finalize()
    if missing_global_state_for_images:
        print(
            "WARNING: {} frames had no global_state; generated topdown images from map+agents only, "
            "without ports/obstacles/queue slots. Regenerate expert logs with include_global_state=True "
            "for pi05 training.".format(missing_global_state_for_images),
            flush=True,
        )
