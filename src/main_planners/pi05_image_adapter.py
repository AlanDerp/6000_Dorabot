class Pi05ImageAdapter:
    ...
    def to_image(self, ego_agent, global_state):
        ...
        for port in global_state.get("loading_ports", []):
            ...
            self._port_guides(image, port, width, height, (0.45, 0.72, 1.00), (0.02, 0.25, 0.90))
        for port in global_state.get("unloading_ports", []):
            self._rect(image, port, width, height, (0.15, 0.70, 0.35))
            self._port_guides(image, port, width, height, (0.52, 0.92, 0.62), (0.00, 0.48, 0.18))
    ...
    def _port_guides(self, image, port, width, height, queue_color, operation_color):
        for slot in port.get("queue_slots", []):
            self._circle(image, slot["x"], slot["y"], width, height, 2, queue_color)
        entry = port.get("entry_point")
        if entry is not None:
            self._circle(image, entry["x"], entry["y"], width, height, 3, queue_color)
        operation = port.get("operation_zone")
        if operation is not None:
            self._circle(image, operation["x"], operation["y"], width, height, 3, operation_color)
